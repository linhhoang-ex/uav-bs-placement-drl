...
def generate_traffic_ON_OFF(n_slots=1000, 
    ...
    ON = True; OFF = False         # state = 1 -> ON and vice versa
    ...
    traffic_type[0] = np.random.choice(n_types) if traffic_state[0] == ON else -1 
    ...
    while t_stop < n_slots:
        ...
        if t_stop < n_slots: 
            t_start = t_stop 
            traffic_state[t_start] = ~np.array(traffic_state[t_stop-1], dtype=bool)
            traffic_type[t_start] = np.random.choice(n_types) if traffic_state[t_start] == ON else -1 
...
if __name__ == "__main__":
    ...
    traffic_arrival = generate_traffic_ON_OFF(n_slots=n_slots, 
