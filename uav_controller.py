...
class UAV_Movement_Controller():
    ...
    def preprocess_user_statistics(self, user_locations, user_statistics, normalization_coeff, average=False):
        ...
        for id, statistic in enumerate(user_statistics):
            heatmap_all_statistics[:,:,id] = self.gen_heatmap(x_locations=user_locations[0], y_locations=user_locations[1], val=statistic, norm_val=normalization_coeff[id])
            if average==True: 
                heatmap_all_statistics[:,:,id] = heatmap_all_statistics[:,:,id] / np.where(hm_loc2<1,1,hm_loc2)   # average over all active users, avoid dividing by 0 
    ...
    def make_movement_decision(self, uav_location, user_locations, user_statistics, normalization_coeff, mask, incoming_traffic_Mb_masked, bw_alloc_all):
        '''
        ...
        user_heatmaps = self.preprocess_user_statistics(user_locations=user_locations, user_statistics=user_statistics, normalization_coeff=normalization_coeff, average=False)
        ...
        dnn_prediction = self.model.predict([tf.expand_dims(uav_location_scale, axis=0), 
                                             tf.expand_dims(user_heatmaps, axis=0)], verbose=0)    
        ...
        for decision_id in range(self.n_decisions): 
            fval = self.evaluate_decision(uav_location, user_locations, user_statistics, ready_decisions[decision_id], mask, incoming_traffic_Mb_masked, bw_alloc_all)
    ...
    def evaluate_decision(self, uav_location, user_locations, user_statistics, ready_decision, mask, incoming_traffic_Mb_masked, bw_alloc_all): 
        '''
        Parameters: 
        ...
        for id in range(n_users):
            ...
            ch_capacity_Mb[id] = self.cal_channel_capacity_Mb(channel_gain=ch_gain, alpha=bw_alloc_all[id])   # equal bandwidth allocation 
        ...
        qlen_new = downlink_load_Mb - ch_capacity_Mb; qlen_new = np.where(qlen_new>0, qlen_new, 0)          # shape=(n_users,)
        mos = np.zeros(shape=(n_users,))
        for uid in range(0,n_users):
            mos[uid] = self.mos_func(qlen_new[uid], incoming_traffic_Mb_masked[uid]) if (mask[uid]==True) else np.nan 
